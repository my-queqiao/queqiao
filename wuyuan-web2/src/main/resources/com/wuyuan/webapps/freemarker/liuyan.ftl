...
<script>	
	function leave(){
		...
		//保存留言
		$.post('${request.contextPath!}/liuyan','userName=${userName}'+'&liuyan='+ly,
				function(json){
					if(json.result == true){
						alert(json.msg);
						//刷新页面
						location.href="${request.contextPath!}/toLiuyan?userName=${userName}"
					}else{
						alert("留言失败");
					}
				}	
				...
				function(json){
					if(json.result == true){
						...
						for(var i = 0;i<lys.length;i++){
							...
							$("#leaveMsg").append("<li style='color:;'>"+lys[i].userName+" : "+lys[i].liuyan+"    "+lys[i].timeString+"</li>");
						}
